...
import numpy as np
from utils import torch_stdct, torch_istdct


# Unofficial implementation of BSRNN from "Music Source Separation with Band-Split RNN"
# Uses STDCT instead of STFT
...
class BSRNN(nn.Module):
    def __init__(self,
        ...
        ### Modules
        self.bandsplit_module = BandSplitModule(sr=sr, n_fft=n_fft, channels=channels, fc_dim=fc_dim, bands=bands,
                                                num_subbands=num_subbands)
        self.bandseq_module = nn.Sequential(
            *[BandSeqModelingModule(channels=channels, fc_dim=fc_dim, group=group, num_subbands=num_subbands) for _ in
              range(num_band_seq_module)]
        )
        self.maskest_module = MaskEstimationModule(sr=sr, n_fft=n_fft, channels=channels, fc_dim=fc_dim, bands=bands,
                                                   num_subbands=num_subbands)
    ...
    def forward(self, wav):
        ...
        spec = torch.stft(wav, n_fft=self.n_fft, hop_length=self.hop_length,
                          ...
                          return_complex=False)  # spec: ((b c), f, t, 2)
        ...
        est_wav = torch.istft(est_cspec, n_fft=self.n_fft, hop_length=self.hop_length,
                              window=torch.hann_window(self.n_fft).to(wav.device))  # ((b c) t)
    ...
    def __init__(self,
        ...
        self.layer_list = nn.ModuleList([
            nn.Sequential(
                ...
                Rearrange('b f t c -> b t (f c)', c=2 * self.channels),  # 2: ri
                nn.LayerNorm(band_interval * self.channels * 2),  # with Rearrange('b f t c -> b t (f c)')
                # Rearrange('b f t c -> b t (f c)', c=2*channels),
                nn.Linear(2 * self.channels * band_interval, self.channels * fc_dim),
    ...
    def __init__(self,
        ...
        self.layer_list = nn.ModuleList([
            nn.Sequential(
                ...
                nn.Tanh(),
                nn.Linear(hidden_dim, band_interval * 2 * self.channels),
